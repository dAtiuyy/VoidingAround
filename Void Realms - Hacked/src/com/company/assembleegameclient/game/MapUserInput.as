...
public class MapUserInput {
    ...
    private function onKeyDown(_arg_1:KeyboardEvent):void {
        ...
        switch (_arg_1.keyCode) {
            case Parameters.data_["tpKey"]:
                var _local_17:* = this.gs_.map.pSTopW(this.gs_.map.mouseX, this.gs_.map.mouseY);
                if (_local_3.isValidPosition(_local_17.x, _local_17.y))
                {
                    _local_3.moveTo(_local_17.x, _local_17.y);
                }
                break;
            case  Parameters.data_["tqKey"]:
                    var _quest:GameObject = this.gs_.map.quest_.getObject(0, true);
                    if(_quest.getName().indexOf("hermit") > 0){
                        _local_3.moveTo(_quest.x_ + 5, _quest.y_);
                    } else {
                        _local_3.moveTo(_quest.x_, _quest.y_);
                    }
                break;
