...
public class EquipmentToolTip extends ToolTip {
    ...
    private function addTierText():void {
        ...
        if (((((_local_1) && (_local_2))) && (_local_3))) {
            ...
            if (_local_4) {
                var _local_5:uint = this.objectXML.Tier.@color != undefined ? this.objectXML.Tier.@color : 0xFFFFFF;
                var _local_6:Boolean = this.objectXML.Tier.@color != undefined;
                var tier = Number(this.objectXML.Tier);
                //<Tier>L<Tier> = L tier with orange tier/title text
                //<Tier color=0xFF0000>L<Tier> = L tier with red tier/title text
                //<Tier color=0xFF00FF>Staff<Tier> = Staff tier with magenta tier text
                //<Tier color=0xFF0000>16<Tier> = T16 tier with red tier text
                switch (this.objectXML.Tier) {
                    case "L":
                        _local_6 ? this.tierText.setColor(_local_5) : this.tierText.setColor(0xFFFF00);
                        _local_6 ? this.titleText.setColor(_local_5) : this.titleText.setColor(0xFFFF00);
                        this.tierText.setStringBuilder(new LineBuilder().setParams("L"));
                        break;
                    default:
                        if(isNaN(this.objectXML.Tier)){
                            this.tierText.setStringBuilder(new LineBuilder().setParams(this.objectXML.Tier));
                        } else {
                            this.tierText.setStringBuilder(new LineBuilder().setParams(TextKey.TIER_ABBR, {"tier": this.objectXML.Tier}));
                        }
                        this.tierText.setColor(_local_5);
                        break;
                }
            }
            else {
