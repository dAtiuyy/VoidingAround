...
public class Player extends Character {
    ...
    public var critRate_:int = 0;
    public var critPower_:int = 0;
    public var critRateBoost_:int = 0;
    public var critPowerBoost_:int = 0;
    public var critRateMax_:int = 0;
    public var critPowerMax_:int = 0;
    ...
    public var startTime:Number = getTimer();
    public var HPS:Number = 0;

    private function checkHPS():void
    {
        var currentTime:Number = (getTimer() - startTime) / 1000;
        HPS = Parameters.data_["Hits"]

        if (currentTime > 1)
        {
            Parameters.data_["HPS"] = Math.floor(((HPS/currentTime)*10.0)/10.0);
            startTime = getTimer();
            HPS = 0;
            Parameters.data_["Hits"] = 0;
        }
    }
    ...
    public function Player(_arg_1:XML) {
        ...
        this.critRateMax_ = int(_arg_1.CritRate.@max);
        this.critPowerMax_ = int(_arg_1.CritPower.@max);
    ...
    public static function fromPlayerXML(_arg_1:String, _arg_2:XML):Player {
        ...
        _local_5.critRate_ = int(_arg_2.CritRate);
        _local_5.critPower_ = int(_arg_2.CritPower);
        return (_local_5);
    }
    ...
    public function onMove():void {
        ...
        if (_local_1.props_.sinking_ && !Parameters.data_.NoSink) {
            ...
            sinkLevel_ = 0;
            if (Parameters.data_.NoSink && _local_1.props_.speed_ < 1.0) {
    ...
    override public function draw(_arg_1:Vector.<IGraphicsData>, _arg_2:Camera, _arg_3:int):void {
        checkHPS();
        if(this.objectId_ != map_.player_.objectId_ && Parameters.data_.hidePlayers) return;
    ...
    private function getMoveSpeed():Number {
        ...
        _local_1 *= this.moveMultiplier_;
        if(Parameters.data_.msmToggle){
            return _local_1 * Parameters.data_.msm;
        } else {
            return _local_1;
        }
    }
    ...
    public function attackFrequency():Number {
        ...
        if(Parameters.data_.dexMod && Parameters.data_.masterDex){
            _local_1 *= Parameters.data_.RoF;
        }
        return (_local_1);
    }
